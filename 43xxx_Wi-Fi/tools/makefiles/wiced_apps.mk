...
ifeq (1,$(FILESYSTEM_IMAGE_SECURE))
	$(QUIET)$(ECHO) Encrypting Filesystem $(FILESYSTEM_IMAGE)
...
ifeq (1,$(APP0_SECURE))
	$(QUIET)$(ECHO) Encrypting APP0 $(APP0)
	$(call CONV_SLASHES, $(AES128_CBC_ENCRYPTOR) enc $(SFLASH_ENCRYPTION_KEY) $(APP0) $(APP0).enc $(BLOCK_SIZE) $(APP0_SECTOR_START))
	$(QUIET)$(ECHO) Downloading secure APP0 $(APP0).enc @ sector $(APP0_SECTOR_START) address $(APP0_SECTOR_ADDRESS) size $(APP0_SECTOR_COUNT)...
	$(call CONV_SLASHES,$(OPENOCD_FULL_NAME)) -f $(OPENOCD_PATH)$(JTAG).cfg -f $(OPENOCD_PATH)$(HOST_OPENOCD).cfg -f apps/waf/sflash_write/sflash_write.tcl -c "sflash_write_file $(APP0).enc $(APP0_SECTOR_ADDRESS) $(SFLASH_APP_PLATFROM_BUS) 0 $(SFLASH_APP_BCM4390)" -c shutdown $(DOWNLOAD_LOG) 2>&1
else
...
OTA2_IMAGE_DEPENDENCIES	:= $(DCT_IMAGE) $(FILESYSTEM_IMAGE) $(FINAL_APPS_FILE) $(OTA_APP) $(WIFI_FIRMWARE) $(APP0) $(APP1) $(APP2)
...
ota2_manuf_image: ota2_factory_image $(OTA2_IMAGE_DEPENDENCIES) $(DCT_IMAGE) $(DCT_IMAGE_DOWNLOAD_DEPENDENCY) ota2_failsafe
	...
	$(QUIET)$(call WRITE_FILE_APPEND, $(OTA2_IMAGE_MANUFACTURING_CONFIG_FILE) ,$(OTA2_IMAGE_FAILSAFE_APP_AREA_BASE) build/$(OTA2_FAILSAFE_TARGET)/binary/$(OTA2_FAILSAFE_TARGET).stripped.elf)
	$(QUIET)$(call WRITE_FILE_APPEND, $(OTA2_IMAGE_MANUFACTURING_CONFIG_FILE) ,$(OTA2_IMAGE_CURR_LUT_AREA_BASE) $(FINAL_APPS_FILE))
	$(QUIET)$(call WRITE_FILE_APPEND, $(OTA2_IMAGE_MANUFACTURING_CONFIG_FILE) ,$(OTA2_IMAGE_CURR_DCT_1_AREA_BASE) $(FINAL_DCT_FILE))
	$(QUIET)$(call WRITE_FILE_APPEND, $(OTA2_IMAGE_MANUFACTURING_CONFIG_FILE) ,$(OTA2_IMAGE_CURR_OTA_APP_AREA_BASE) $(OTA_APP))
	$(QUIET)$(call WRITE_FILE_APPEND, $(OTA2_IMAGE_MANUFACTURING_CONFIG_FILE) ,$(OTA2_IMAGE_CURR_FS_AREA_BASE) $(FILESYSTEM_IMAGE))
	$(QUIET)$(call WRITE_FILE_APPEND, $(OTA2_IMAGE_MANUFACTURING_CONFIG_FILE) ,$(OTA2_IMAGE_CURR_APP0_AREA_BASE) $(APP0))
ifneq ($(APP1),)
