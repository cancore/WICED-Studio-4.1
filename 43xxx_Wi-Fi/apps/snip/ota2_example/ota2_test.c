...
wiced_result_t ota2_test_get_update(ota2_data_t* player)
{
    ...
    {
        ota2_test_stop_timed_update(player->ota2_bg_service);
    }
    return result;
}
...
static void ota2_test_mainloop(ota2_data_t *player)
{
    ...
    while (player->tag == PLAYER_TAG_VALID)
    {
        ...
        if (events & PLAYER_EVENT_DISCONNECT_AP)
        {
            ...
            result = wiced_ota2_network_down(&player->saved_network_parameters);
            if (result != WICED_SUCCESS)
            {
        ...
        if (events & PLAYER_EVENT_CONNECT_OTA2_AP)
        {
            ...
            {
                /* disconnect from current AP and connect to OTA2 update AP */
                result = wiced_ota2_network_up(&player->dct_wifi->stored_ap_list[1], &player->saved_network_parameters);
                if (result != WICED_SUCCESS)
                {
        ...
        if (events & PLAYER_EVENT_CONNECT_DEFAULT_AP)
        {
            ...
            result = wiced_ota2_network_up(&player->dct_wifi->stored_ap_list[0], &player->saved_network_parameters);
            if (result != WICED_SUCCESS)
            {
