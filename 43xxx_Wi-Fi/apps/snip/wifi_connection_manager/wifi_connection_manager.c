...
typedef enum
{
    ...
    CONNECTION_EVENT_WPS_ENROLLEE_LEGACY     = (1 << 2),
    CONNECTION_EVENT_P2P_GROUP_OWNER         = (1 << 3),
    CONNECTION_EVENT_P2P_GROUP_CLIENT_LEGACY = (1 << 4),
} CONNECTION_EVENTS_T;
...
static void app_main_loop(void)
{
    ...
    while(WICED_TRUE)
    {
        ...
        if (events & CONNECTION_EVENT_WPS_ENROLLEE_LEGACY)
        {
            result = connection_launch(CONNECTION_WPS_ENROLLEE_LEGACY);
        ...
        if (events & CONNECTION_EVENT_P2P_GROUP_CLIENT_LEGACY)
        {
            /* Register a result callback for p2p connection */
            connection_register_p2p_result_callback(app_p2p_result_callback);
            connection_launch(CONNECTION_P2P_GC_LEGACY);
        }
    }
}
...
static wiced_result_t button_event_handler(app_service_action_t action)
{
    ...
    switch( action )
    {
        ...
        case ACTION_BACKWARD_SHORT_RELEASE:
            result = wiced_rtos_set_event_flags(&connection_events, CONNECTION_EVENT_P2P_GROUP_CLIENT_LEGACY);
            break;
        ...
        case ACTION_MULTI_FUNCTION_SHORT_RELEASE:
            result = wiced_rtos_set_event_flags(&connection_events, CONNECTION_EVENT_WPS_ENROLLEE_LEGACY);
            break;
        default:
            break;
    }
