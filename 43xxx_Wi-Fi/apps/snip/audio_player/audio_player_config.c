 ...
 ******************************************************/
static wiced_result_t audio_player_save_app_dct(audio_player_t* player);
static void audio_player_print_app_info(audio_player_t* player);
static void audio_player_print_network_info(audio_player_t* player);
static void audio_player_print_wifi_info(audio_player_t* player);
static wiced_bool_t audio_player_encode_speaker_channel(int argc, char *argv[], AUDIO_CHANNEL_MAP_T* channel);
...
static wiced_result_t audio_player_save_app_dct(audio_player_t* player)
{
    return wiced_dct_write( (void*)player->dct_app, DCT_APP_SECTION, 0, sizeof(audio_player_app_dct_t) );
}

/* Prints null-terminated speaker channel type in the provided buffer */
static void audio_player_channel_name_get_text(uint32_t channel, char *buff, int buff_len)
{
    ...
    switch( cmd )
    {
        ...
        case CONFIG_CMD_SPEAKER_CHANNEL:
        {
            ...
            if (argc > 2)
            {
                ...
                if( audio_player_encode_speaker_channel(argc, argv, &new_speaker_channel) == WICED_TRUE)
                {
                    audio_player_channel_name_get_text(player->dct_app->channel, buff, sizeof(buff));
                    audio_player_channel_name_get_text(new_speaker_channel, buff2, sizeof(buff2));
        ...
        case CONFIG_CMD_SAVE:
            if (app_dct_dirty != 0)
            {
                ...
                audio_player_save_app_dct(player);
                app_dct_dirty = 0;
                audio_player_print_app_info(player);
            }
            break;
...
static void audio_player_print_wifi_info(audio_player_t* player)
{
    ...
    audio_player_get_mac_addr_text(&player->dct_wifi->mac_address, mac_str, sizeof(mac_str));
...
static void audio_player_print_current_info(audio_player_t* player)
{
    uint32_t channel;
...
void audio_player_config_print_info(audio_player_t* player)
{
    WPRINT_APP_INFO(("\r\nConfig Info: * = dirty\r\n"));
    audio_player_print_app_info(player);
    audio_player_print_network_info(player);
    audio_player_print_wifi_info(player);
    audio_player_print_current_info(player);
    WPRINT_APP_INFO(("\r\n"));
}

static wiced_result_t audio_player_config_load_dct_wifi(audio_player_t* player)
{
...
static wiced_result_t audio_player_config_load_dct_network(audio_player_t* player)
{
    wiced_result_t result;
...
wiced_result_t audio_player_config_init(audio_player_t* player)
{
    wiced_result_t result;

    /* wifi */
    result = audio_player_config_load_dct_wifi(player);

    /* network */
    result |= audio_player_config_load_dct_network(player);

    /* App */
    result |= wiced_dct_read_lock( (void**)&player->dct_app, WICED_TRUE, DCT_APP_SECTION, 0, sizeof( audio_player_app_dct_t ) );

    return result;
}

static wiced_result_t audio_player_config_unload_dct_wifi(audio_player_t* player)
{
...
static wiced_result_t audio_player_config_unload_dct_network(audio_player_t* player)
{
    wiced_result_t result = WICED_SUCCESS;
...
wiced_result_t audio_player_config_deinit(audio_player_t* player)
{
    wiced_result_t result;

    result = wiced_dct_read_unlock(player->dct_app, WICED_TRUE);

    result |= audio_player_config_unload_dct_network(player);
    result |= audio_player_config_unload_dct_wifi(player);


    return result;
}

wiced_result_t audio_player_config_reload_dct_wifi(audio_player_t* player)
{
    wiced_result_t result;
    result =  audio_player_config_unload_dct_wifi(player);
    result |=  audio_player_config_load_dct_wifi(player);
...
wiced_result_t audio_player_config_reload_dct_network(audio_player_t* player)
{
    wiced_result_t result;
    result =  audio_player_config_unload_dct_network(player);
    result |=  audio_player_config_load_dct_network(player);
