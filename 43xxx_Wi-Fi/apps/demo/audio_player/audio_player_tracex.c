...
static int audio_player_tracex_send(int argc, char *argv[])
{
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "Failed to get TraceX configuration\n");
        return ERR_UNKNOWN;
    }
    ...
    if (argc > 2)
    {
        ...
        {
            wiced_log_msg(WICED_LOG_ERR, "Invalid TCP server IP address\n");
    ...
    wiced_log_msg(WICED_LOG_INFO, "Sending TraceX buffer to TCP server (%lu.%lu.%lu.%lu:%u)\n",
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "Cannot send TraceX buffer while TraceX is enabled\n");
        return ERR_UNKNOWN;
    }
    else if (result == WICED_NOT_CONNECTED)
    {
        wiced_log_msg(WICED_LOG_ERR, "Failed to connect to TCP server\n");
        return ERR_UNKNOWN;
    }
    else if (result != WICED_SUCCESS)
    {
        wiced_log_msg(WICED_LOG_ERR, "Failed to send TraceX buffer\n");
        return ERR_UNKNOWN;
    }
...
static int audio_player_tracex_status(void)
{
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "Failed to get TraceX status\n");
        return ERR_UNKNOWN;
    }
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "Failed to get TraceX filters\n");
        return ERR_UNKNOWN;
    }
...
static int audio_player_tracex_disable(void)
{
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "TraceX not running\n");
        return ERR_CMD_OK;
    }
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "Failed to disable TraceX\n");
        return ERR_UNKNOWN;
    }
...
static int audio_player_tracex_enable(int argc, char *argv[])
{
    ...
    wiced_log_msg(WICED_LOG_INFO, "Enabling TraceX\n");
    result = wiced_tracex_enable(&config);

    if (result == WICED_NOT_CONNECTED)
    {
        wiced_log_msg(WICED_LOG_ERR, "Failed to connect to TCP server (%lu.%lu.%lu.%lu:%u)\n",
                       (GET_IPV4_ADDRESS(config.tcp_server.ip) >> 24) & 0xFF, (GET_IPV4_ADDRESS(config.tcp_server.ip) >> 16) & 0xFF,
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "TraceX was already enabled\n");
        return ERR_UNKNOWN;
    }
    else if (result != WICED_SUCCESS)
    {
        wiced_log_msg(WICED_LOG_ERR, "Failed to enable TraceX\n");
        return ERR_UNKNOWN;
    }
...
int audio_player_tracex_command(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        wiced_log_msg(WICED_LOG_ERR, "No TraceX command specified\n");
        return ERR_CMD_OK;
    }
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "Unrecognized TraceX command: %s\n", argv[1]);
        return ERR_CMD_OK;
    }
