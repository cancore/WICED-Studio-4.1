...
#define GET_ON_OFF_VALUE(idx, var)                                                                          \
    ...
    {                                                                                                       \
        wiced_log_msg(WICED_LOG_ERR, "Bad value for on/off setting: %s\n", argv[idx]);                      \
        break;                                                                                              \
    }
    ...
    switch (cmd)
    {
        ...
        case CONFIG_CMD_AUDIO_DEVICE_TX:
        {
            ...
            if (argc > 2)
            {
                ...
                if ((device_index >= 0) && (device_index < PLATFORM_AUDIO_NUM_OUTPUTS))
                {
                    ...
                    {
                        wiced_log_msg(WICED_LOG_DEBUG0, "Audio device TX ID: 0x%x -> 0x%x %s\n", dct_tables->dct_app->audio_device_tx, device,
        ...
        case CONFIG_CMD_HTTP_BUFFER_NUM:
            if (argc > 2)
            {
                ...
                if (arg <= 0)
                {
                    wiced_log_msg(WICED_LOG_ERR, "Bad value for HTTP buffer num: %d\n", arg);
                    break;
                }
                if (dct_tables->dct_app->http_buffer_num != arg)
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "CONFIG_CMD_HTTP_BUFFER_NUM: %d -> %d\n", dct_tables->dct_app->http_buffer_num, arg);
                    dct_tables->dct_app->http_buffer_num = arg;
                    app_dct_dirty |= APP_DCT_HTTP_BUFFER_NUM_DIRTY;
                }
            }
            wiced_log_msg(WICED_LOG_NOTICE, "HTTP buffer num: %d\n", dct_tables->dct_app->http_buffer_num);
            break;
        ...
        case CONFIG_CMD_AUDIO_BUFFER_NUM:
            if (argc > 2)
            {
                ...
                if (arg <= 0)
                {
                    wiced_log_msg(WICED_LOG_ERR, "Bad value for audio buffer num: %d\n", arg);
                    break;
                }
                if (dct_tables->dct_app->audio_buffer_num != arg)
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "CONFIG_CMD_AUDIO_BUFFER_NUM: %d -> %d\n", dct_tables->dct_app->audio_buffer_num, arg);
                    dct_tables->dct_app->audio_buffer_num = arg;
                    app_dct_dirty |= APP_DCT_AUDIO_BUFFER_NUM_DIRTY;
                }
            }
            wiced_log_msg(WICED_LOG_NOTICE, "Audio buffer num: %d\n", dct_tables->dct_app->audio_buffer_num);
            break;
        ...
        case CONFIG_CMD_AUDIO_BUFFER_SIZE:
            if (argc > 2)
            {
                ...
                if (arg <= 0)
                {
                    wiced_log_msg(WICED_LOG_ERR, "Bad value for audio buffer size: %d\n", arg);
                    break;
                }
                if (dct_tables->dct_app->audio_buffer_size != arg)
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "CONFIG_CMD_AUDIO_BUFFER_SIZE: %d -> %d\n", dct_tables->dct_app->audio_buffer_size, arg);
                    dct_tables->dct_app->audio_buffer_size = arg;
                    app_dct_dirty |= APP_DCT_AUDIO_BUFFER_SIZE_DIRTY;
                }
            }
            wiced_log_msg(WICED_LOG_NOTICE, "Audio buffer size: %d\n", dct_tables->dct_app->audio_buffer_size);
            break;
        ...
        case CONFIG_CMD_AUDIO_PERIOD_SIZE:
            if (argc > 2)
            {
                ...
                if (arg <= 0)
                {
                    wiced_log_msg(WICED_LOG_ERR, "Bad value for audio period size: %d\n", arg);
                    break;
                }
                if (dct_tables->dct_app->audio_period_size != arg)
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "CONFIG_CMD_AUDIO_PERIOD_SIZE: %d -> %d\n", dct_tables->dct_app->audio_period_size, arg);
                    dct_tables->dct_app->audio_period_size = arg;
                    app_dct_dirty |= APP_DCT_AUDIO_PERIOD_SIZE_DIRTY;
                }
            }
            wiced_log_msg(WICED_LOG_NOTICE, "Audio period buffer size: %d\n", dct_tables->dct_app->audio_period_size);
            break;
        ...
        case CONFIG_CMD_HTTP_THRESHOLD_HIGH:
            if (argc > 2)
            {
                ...
                if (arg < 0)
                {
                    wiced_log_msg(WICED_LOG_ERR, "Bad value for HTTP threshold high: %d\n", arg);
                    break;
                }
                if (dct_tables->dct_app->http_threshold_high != arg)
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "CONFIG_CMD_HTTP_THRESHOLD_HIGH: %d -> %d\n", dct_tables->dct_app->http_threshold_high, arg);
                    dct_tables->dct_app->http_threshold_high = arg;
                    app_dct_dirty |= APP_DCT_HTTP_THRESHOLD_HIGH_DIRTY;
                }
            }
            wiced_log_msg(WICED_LOG_NOTICE, "HTTP threshold high: %d\n", dct_tables->dct_app->http_threshold_high);
            break;
        ...
        case CONFIG_CMD_HTTP_THRESHOLD_LOW:
            if (argc > 2)
            {
                ...
                if (arg < 0)
                {
                    wiced_log_msg(WICED_LOG_ERR, "Bad value for HTTP threshold low: %d\n", arg);
                    break;
                }
                if (dct_tables->dct_app->http_threshold_low != arg)
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "CONFIG_CMD_HTTP_THRESHOLD_LOW: %d -> %d\n", dct_tables->dct_app->http_threshold_low, arg);
                    dct_tables->dct_app->http_threshold_low = arg;
                    app_dct_dirty |= APP_DCT_HTTP_THRESHOLD_LOW_DIRTY;
                }
            }
            wiced_log_msg(WICED_LOG_NOTICE, "HTTP threshold low: %d\n", dct_tables->dct_app->http_threshold_low);
            break;
        ...
        case CONFIG_CMD_HTTP_BUFFER_PREROLL:
            if (argc > 2)
            {
                ...
                if (arg < 0)
                {
                    wiced_log_msg(WICED_LOG_ERR, "Bad value for HTTP buffer preroll: %d\n", arg);
                    break;
                }
                if (dct_tables->dct_app->http_buffer_preroll != arg)
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "CONFIG_CMD_HTTP_BUFFER_PREROLL: %d -> %d\n", dct_tables->dct_app->http_buffer_preroll, arg);
                    dct_tables->dct_app->http_buffer_preroll = arg;
                    app_dct_dirty |= APP_DCT_HTTP_BUFFER_PREROLL_DIRTY;
                }
            }
            wiced_log_msg(WICED_LOG_NOTICE, "HTTP buffer preroll: %d\n", dct_tables->dct_app->http_buffer_preroll);
            break;
