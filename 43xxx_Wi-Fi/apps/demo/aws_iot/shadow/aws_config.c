...
static int32_t process_upgrade_chunk( const char* url_parameters, const char* url_query_string, wiced_http_response_stream_t* stream, void* arg, wiced_http_message_body_t* http_data )
{
    ...
    printf("[%s] [%d] received_offset [%d] file_size [%d] expected_offset [%d]\n", __FUNCTION__, __LINE__, (int)received_offset, (int)file_size, (int)expected_offset);
    ...
    printf("http_data->message_data_length [%d]\n", (int)http_data->message_data_length);
...
wiced_result_t aws_configure_device(void)
{
    ...
    if ( device_configured != WICED_TRUE )
    {
...
#ifdef USE_HTTPS
        http_server = MALLOC_OBJECT("http server", wiced_https_server_t);
...
#ifdef USE_HTTPS
        {
            ...
            wiced_https_server_start(https_server, HTTPS_PORT, max_sockets, https_pages, &tls_identity, WICED_STA_INTERFACE, DEFAULT_URL_PROCESSOR_STACK_SIZE );
        ...
        /* Cleanup HTTP server */
#ifdef USE_HTTPS
        wiced_https_server_stop(https_server);
#else
