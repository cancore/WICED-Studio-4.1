...
wiced_result_t bt_audio_configure_device( bt_audio_config_t* p_audio_config, bt_audio_device_type_t device_type )
{
    ...
    if(device_type == BT_AUDIO_DEVICE_PLAYER)
        result = wiced_audio_init(PLATFORM_DEFAULT_AUDIO_OUTPUT, &device->bluetooth_audio_session_handle, BT_AUDIO_DEFAULT_PERIOD_SIZE);
    else //recorder
        result = wiced_audio_init(PLATFORM_DEFAULT_AUDIO_INPUT, &device->bluetooth_audio_session_handle, BT_AUDIO_DEFAULT_PERIOD_SIZE);
