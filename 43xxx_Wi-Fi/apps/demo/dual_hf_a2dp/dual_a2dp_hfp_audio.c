 ...
 *
 * This application aims to demonstrate Dual A2DP, Dual HFP functionalities.
 * Few notes:
 * DUT/WICED board can connect to maximum of two remote devices at the same time.
 * DUT can have maximum of two HFP, two A2DP and two AVRCP active connection at the same time.
 * Dual A2DP > When DUT is streaming from AV SRC1, on starting AV stream from SRC2 will suspend the streaming from AV SRC1 and resume streaming from AV SRC2 only after receiving AV SRC1 suspend confirmation.
 *
 */
