...
wiced_result_t apollo_debug_create_tcp_data_socket(wiced_ip_address_t* ip_addr, int port)
{
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "TCP socket creation failed\n");
        return WICED_ERROR;
    }
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "Unable to connect to the debug server!\n");
        return result;
    }
...
wiced_result_t apollo_debug_send_tcp_data(uint8_t* data, int datalen)
{
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "Too much data %d (max %d)\n", datalen, TCP_PACKET_MAX_DATA_LENGTH);
        return WICED_ERROR;
    }
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "TCP packet creation failed\n");
        return WICED_ERROR;
    }
    ...
    if (result != WICED_SUCCESS)
    {
        wiced_log_msg(WICED_LOG_ERR, "TCP packet send failed (%d)\n", result);
