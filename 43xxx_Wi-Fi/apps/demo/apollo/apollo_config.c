...
static wiced_bool_t apollo_encode_speaker_channel(int argc, char *argv[], APOLLO_CHANNEL_MAP_T* channel)
{
    ...
    for (arg_index = 2; arg_index < argc; arg_index++)
    {
        ...
        wiced_log_msg(WICED_LOG_DEBUG0, "\r\n   chan: 0x%08x\r\n", new_speaker_channel_map);
    }
    ...
    switch(cmd)
    {
        ...
        case CONFIG_CMD_BUFFERING_MS:
            if (argc > 2)
            {
                ...
                {
                    wiced_log_msg(WICED_LOG_ERR, "Error: out of range min:%d  max:%d\r\n", APOLLO_BUFFERING_MS_MIN, APOLLO_BUFFERING_MS_MAX);
                    break;
                }
        ...
        case CONFIG_CMD_RMC_SSID:
        {
            if (argc > 2)
            {
                ...
                {
                    wiced_log_msg(WICED_LOG_ERR, "Error: too long (max: %d)\r\n", sizeof(dct_tables->dct_app->rmc_info.ssid_name));
                    break;
                }
                    ...
                    strncmp((char*)dct_tables->dct_app->rmc_info.ssid_name, argv[2], dct_tables->dct_app->rmc_info.ssid_length)  != 0)
                {
                    ...
                    wiced_log_msg(WICED_LOG_ERR, "WARNING: If RMC connection is up, you must reboot after changing RMC configuration.\r\n");
                }
            }
            wiced_log_msg(WICED_LOG_NOTICE, "RMC Network name: (%d) %.*s\r\n", dct_tables->dct_app->rmc_info.ssid_length,
        ...
        case CONFIG_CMD_RMC_PASSPHRASE:
        {
            if (argc > 2)
            {
                ...
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "passphrase: %.*s -> %s \r\n",
            ...
            wiced_log_msg(WICED_LOG_NOTICE, "RMC Network passphrase: %.*s \r\n",
        ...
        case CONFIG_CMD_RMC_CHANNEL:
        {
            ...
            if (argc > 2)
            {
                ...
                if (apollo_get_channel_band(new_channel, &new_band) != WICED_TRUE)
                {
                    ...
                    wiced_log_msg(WICED_LOG_ERR, "Error: %d not a valid channel.\r\n", new_channel);
                    break;
                }
                if ((dct_tables->dct_app->rmc_info.channel != new_channel) ||
                    (dct_tables->dct_app->rmc_info.band != new_band))
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "channel: %d -> %d\r\n  Band: %d -> %d \r\n",
            ...
            wiced_log_msg(WICED_LOG_NOTICE, "RMC Network channel:%d  band:%s\r\n", dct_tables->dct_app->rmc_info.channel,
        ...
        case CONFIG_CMD_RMC_SECURITY:
            if (argc > 2)
            {
                ...
                {
                    wiced_log_msg(WICED_LOG_ERR, "Error: unknown security type: %s\r\n", (argv[2][0] != 0) ? argv[2] : "");
                    break;
                }
                if (dct_tables->dct_app->rmc_info.security != new_sec_type)
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "network security %s -> %s\r\n",
            ...
            wiced_log_msg(WICED_LOG_NOTICE, "RMC Network security type: %s\r\n", apollo_security_type_get_name(dct_tables->dct_app->rmc_info.security));
            break;
        ...
        case CONFIG_CMD_SPEAKER_NAME:
        {
            ...
            if (argc > 2)
            {
                ...
                if (strcmp(name, argv[2]) != 0)
                {
                    ...
                    wiced_log_msg(WICED_LOG_DEBUG0, "Speaker name: %s -> %s\r\n", name, argv[2]);
            ...
            wiced_log_msg(WICED_LOG_NOTICE, "Speaker name: %s\r\n", dct_tables->dct_app->speaker_name);
            break;
        }
        ...
        case CONFIG_CMD_SPEAKER_CHANNEL:
        {
            ...
            if (argc > 2)
            {
                ...
                if (apollo_encode_speaker_channel(argc, argv, &new_speaker_channel) == WICED_TRUE)
                {
                    ...
                    {
                        wiced_log_msg(WICED_LOG_DEBUG0, "Speaker channel: (0x%08x) %s -> (0x%08x) %s\r\n",
            ...
            apollo_channel_name_get_text(dct_tables->dct_app->speaker_channel, buff, sizeof(buff));
            wiced_log_msg(WICED_LOG_NOTICE, "Speaker channel: (0x%08x) %s\r\n", dct_tables->dct_app->speaker_channel, buff);
            break;
        }
        ...
        case CONFIG_CMD_THRESHOLD_MS:
            if (argc > 2)
            {
                ...
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "CONFIG_CMD_THRESHOLD_MS: %d -> %d\r\n", dct_tables->dct_app->threshold_ms, ms);
                    dct_tables->dct_app->threshold_ms = ms;
                    app_dct_dirty |= APP_DCT_THRESH_MS_DIRTY;
                }
            }
            wiced_log_msg(WICED_LOG_NOTICE, "Threshold: %d ms\r\n", dct_tables->dct_app->threshold_ms);
            break;
        ...
        case CONFIG_CMD_VOLUME:
            if (argc > 2)
            {
                ...
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "CONFIG_CMD_VOLUME: %d -> %d\r\n", dct_tables->dct_app->volume, volume);
                    dct_tables->dct_app->volume = volume;
                    app_dct_dirty |= APP_DCT_VOLUME_DIRTY;
                }
            }
            wiced_log_msg(WICED_LOG_NOTICE, "Volume: %d\r\n", dct_tables->dct_app->volume);
            break;
        ...
        case CONFIG_CMD_BURST_LENGTH:
        {
            ...
            if (argc > 2)
            {
                ...
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "Burst length: %d -> %d\r\n", dct_tables->dct_app->burst_length, burst_length);
                    dct_tables->dct_app->burst_length = burst_length;
            ...
            {
                wiced_log_msg(WICED_LOG_NOTICE, "Burst length: %d\r\n", dct_tables->dct_app->burst_length);
            }
            else
            {
                wiced_log_msg(WICED_LOG_NOTICE, "Burst length: auto\r\n");
            }
            break;
        }
        ...
        case CONFIG_CMD_AUDIO_DEVICE_RX:
        {
            ...
            if (argc > 2)
            {
                ...
                if ((device_index >= 0) && (device_index < PLATFORM_AUDIO_NUM_INPUTS))
                {
                    ...
                    {
                        wiced_log_msg(WICED_LOG_NOTICE, "Audio device RX ID: 0x%x -> 0x%x\r\n", dct_tables->dct_app->audio_device_rx,
        ...
        case CONFIG_CMD_AUDIO_DEVICE_TX:
        {
            ...
            if (argc > 2)
            {
                ...
                if ((device_index >= 0) && (device_index < PLATFORM_AUDIO_NUM_OUTPUTS))
                {
                    ...
                    {
                        wiced_log_msg(WICED_LOG_NOTICE, "Audio device TX ID: 0x%x -> 0x%x %s\r\n", dct_tables->dct_app->audio_device_tx,
        ...
        case CONFIG_CMD_INPUT_SAMPLE_RATE:
        {
            ...
            if (argc > 2)
            {
                ...
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "Input sample rate: %s KHz-> %s KHz\r\n",
            ...
            wiced_log_msg(WICED_LOG_NOTICE, "Input sample rate: %s KHz\r\n", platform_audio_device_get_sample_rates_string(dct_tables->dct_app->input_sample_rate));
            break;
        }
        ...
        case CONFIG_CMD_CLIENTADDR:
        {
            ...
            if (argc > 2)
            {
                ...
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "Apollo client IP address: %d.%d.%d.%d -> %d.%d.%d.%d\r\n",
            ...
            wiced_log_msg(WICED_LOG_NOTICE, "Apollo client IP address: %d.%d.%d.%d\r\n",
                          (dct_tables->dct_app->clientaddr.ip.v4 >> 24) & 0xFF, (dct_tables->dct_app->clientaddr.ip.v4 >> 16) & 0xFF,
                          (dct_tables->dct_app->clientaddr.ip.v4 >> 8) & 0xFF, dct_tables->dct_app->clientaddr.ip.v4 & 0xFF);
            break;
        }
        ...
        case CONFIG_CMD_RTP_PORT:
            if (argc > 2)
            {
                ...
                if (port < 0)
                {
                    wiced_log_msg(WICED_LOG_ERR, "Error: port %d invalid\r\n", port);
                    break;
                }
                if (dct_tables->dct_app->rtp_port != port)
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "rtp_port: %d -> %d\r\n", dct_tables->dct_app->rtp_port, port);
                    dct_tables->dct_app->rtp_port = port;
                    app_dct_dirty |= APP_DCT_RTP_PORT_DIRTY;
                }
            }
            wiced_log_msg(WICED_LOG_NOTICE, "RTP port: %d\r\n", dct_tables->dct_app->rtp_port);
            break;
        ...
        case CONFIG_CMD_LOG_LEVEL:
            if (argc > 2)
            {
                ...
                if (level < 0)
                {
                    wiced_log_msg(WICED_LOG_ERR, "Error: log level %d invalid\r\n", level);
                    break;
                }
                if (dct_tables->dct_app->log_level != level)
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "log_level: %d -> %d\r\n", dct_tables->dct_app->log_level, level);
                    dct_tables->dct_app->log_level = level;
                    app_dct_dirty |= APP_DCT_LOG_LEVEL_DIRTY;
                }
            }
            wiced_log_msg(WICED_LOG_NOTICE, "Log level: %d\r\n", dct_tables->dct_app->log_level);
            break;
        ...
        case CONFIG_CMD_OTA2_URI:
        {
            if (argc > 2)
            {
                ...
                {
                    wiced_log_msg(WICED_LOG_DEBUG0, "OTA2 URI: %s -> %s \r\n",
            ...
            wiced_log_msg(WICED_LOG_NOTICE, "OTA2 Default Update URI: %s \r\n", dct_tables->dct_app->ota2_default_update_uri);
            break;
        }
        break;
#endif
        case CONFIG_CMD_SAVE:
            if (app_dct_dirty != 0)
            {
                wiced_log_msg(WICED_LOG_NOTICE, "Saving App DCT:\r\n");
        ...
        default:
            wiced_log_msg(WICED_LOG_ERR, "Unrecognized config command: %s\r\n", (argv[1][0] != 0) ? argv[1] : "");
            apollo_config_command_help();
            break;
    }
...
static void apollo_print_app_info(apollo_dct_collection_t* dct_tables)
{
    ...
    wiced_log_printf("         log level: %d (currently %d) %c\r\n", dct_tables->dct_app->log_level, wiced_log_get_level(), DIRTY_CHECK(app_dct_dirty, APP_DCT_LOG_LEVEL_DIRTY));
...
static wiced_result_t apollo_config_load_dct_bluetooth(apollo_dct_collection_t* dct_tables)
{
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "Can't get BT configuration!\r\n");
    }
    return result;
}
#endif
...
static wiced_result_t apollo_config_unload_dct_bluetooth(apollo_dct_collection_t* dct_tables)
{
    ...
    if ((dct_tables != NULL) && (dct_tables->dct_bt != NULL))
    {
        ...
        {
            wiced_log_msg(WICED_LOG_ERR, "Can't free/release BT configuration !\r\n");
        }
    }
    return result;
}
...
wiced_result_t apollo_config_save(apollo_dct_collection_t* dct_tables)
{
    ...
    if ( result != WICED_SUCCESS )
    {
        wiced_log_msg(WICED_LOG_ERR, "apollo_save_app_dct() failed !\r\n");
    }

    result |= apollo_save_wifi_dct( dct_tables );
    if ( result != WICED_SUCCESS )
    {
        wiced_log_msg(WICED_LOG_ERR, "apollo_save_wifi_dct() failed !\r\n");
    }
    ...
    if ( result != WICED_SUCCESS )
    {
        wiced_log_msg(WICED_LOG_ERR, "apollo_save_network_dct() failed !\r\n");
    }
#endif
    ...
    if ( result != WICED_SUCCESS )
    {
        wiced_log_msg(WICED_LOG_ERR, "apollo_save_bt_dct() failed !\r\n");
    }
#endif
