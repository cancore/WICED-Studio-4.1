...
extern ak4961_device_data_t ak4961_dac;
extern ak4961_device_data_t ak4961_adc;
extern ak4961_device_data_t ak4961_adc_dmic1;
extern spdif_device_data_t spdif;
...
wiced_result_t wiced_audio_configure( wiced_audio_session_ref sh, wiced_audio_config_t* config )
{
    ...
    if ( sh->audio_dev->audio_device_configure )
    {
        ...
        wiced_i2s_spdif_mode_t i2s_spdif_mode = WICED_I2S_SPDIF_MODE_OFF;

        /* Init audio interface, for all codecs the audio interface will be initialised similarly */
        if (sh->audio_dev->audio_device_driver_specific == &ak4961_dac)
        {
            i2s_id = ((ak4961_device_data_t*)sh->audio_dev->audio_device_driver_specific)->data_port;
            i2s_direction = WICED_PLAY_CHANNEL;
        }
        else if (sh->audio_dev->audio_device_driver_specific == &ak4961_adc)
        {
            i2s_id = ((ak4961_device_data_t*)sh->audio_dev->audio_device_driver_specific)->data_port;
            i2s_direction = WICED_RECORD_CHANNEL;
        }
        else if (sh->audio_dev->audio_device_driver_specific == &ak4961_adc_dmic1)
        {
            i2s_id = ((ak4961_device_data_t*)sh->audio_dev->audio_device_driver_specific)->data_port;
            i2s_direction = WICED_RECORD_CHANNEL;
        }
        else if (sh->audio_dev->audio_device_driver_specific == &spdif)
        {
            i2s_id = ((spdif_device_data_t*)sh->audio_dev->audio_device_driver_specific)->data_port;
            i2s_direction  = WICED_RECORD_CHANNEL;
            i2s_spdif_mode = WICED_I2S_SPDIF_MODE_ON;
        }
        ...
        params.i2s_spdif_mode  = i2s_spdif_mode;
...
wiced_result_t wiced_audio_stop( wiced_audio_session_ref sh )
{
    ...
    result = sh->audio_dev->audio_device_stop_streaming( sh->audio_dev->audio_device_driver_specific );
...
wiced_result_t wiced_audio_set_effect(wiced_audio_session_ref sh, uint8_t mode)
{
    ...
    if (( sh->audio_dev->audio_device_set_effect != NULL ) && ( sh->audio_dev->audio_device_driver_specific == &ak4961_dac ))
    {
        result = sh->audio_dev->audio_device_set_effect( sh->audio_dev->audio_device_driver_specific, mode );
    }
    else
    {
...
{
    wiced_i2s_t i2s = sh->i2s_id;
    return wiced_i2s_pll_set_fractional_divider( i2s, value );
}
...
wiced_result_t wiced_audio_update_period_size( wiced_audio_session_ref sh, uint16_t period_size )
{
    ...
    /* Period size cannot exceed device limit. */
    wiced_required_action(((period_size > 0) && (period_size <= I2S_PERIOD_BYTES_MAX)), exit, result = WICED_BADARG);
