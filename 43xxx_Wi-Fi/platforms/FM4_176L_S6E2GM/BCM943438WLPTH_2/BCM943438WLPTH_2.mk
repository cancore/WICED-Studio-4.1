...
NAME := Platform_FM4_176L_S6E2GM_BCM943438WLPTH_2

WLAN_CHIP            := 43438
WLAN_CHIP_REVISION   := A1
WLAN_CHIP_FAMILY     := 4343x
HOST_MCU_FAMILY      := CYFM4
HOST_MCU_VARIANT     := s6e2gm
HOST_MCU_PART_NUMBER := s6e2gmxj
BT_CHIP              := 43438
BT_CHIP_REVISION     := A1
BT_MODE              ?= HCI
...
##TODO: Some flags needs to be removed when the functionality is implemented
# Global defines.
# HSE_VALUE = STM32 crystal frequency = 26MHz (needed to make UART work correctly)
#GLOBAL_DEFINES += HSE_VALUE=26000000
GLOBAL_DEFINES += $$(if $$(NO_CRLF_STDIO_REPLACEMENT),,CRLF_STDIO_REPLACEMENT)
GLOBAL_DEFINES    += WWD_BUS_SDIO_USE_WLAN_SDIO_PULLUPS
GLOBAL_DEFINES    += WICED_PLATFORM_MASKS_BUS_IRQ
#GLOBAL_DEFINES    += WICED_PLATFORM_DOESNT_USE_TEMP_DMA_BUFFER

GLOBAL_DEFINES    += WICED_DISABLE_WATCHDOG
GLOBAL_DEFINES    += WICED_DISABLE_MCU_POWERSAVE
GLOBAL_DEFINES    += WWD_BUS_SDIO_AFTER_RESET_DELAY=1000
GLOBAL_DEFINES    += WWD_DISABLE_SAVE_RESTORE
#GLOBAL_DEFINES    += HIGH_SPEED_SDIO_CLOCK
