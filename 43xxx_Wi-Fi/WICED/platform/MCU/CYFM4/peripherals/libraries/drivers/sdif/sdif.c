                     ...
                     stc_sdif_intern_data_t* pstcSdifInternData)
{
    ...
    {
        /* As per FM4 TRM, Section: 2.17 Normal Interrupt Status Register,
         * The Card Interrupt bit can be cleared to 0 by setting the Card Interrupt Status Enable bit
         * in the Normal Interrupt Status Enable Register to 0.
         */
//        pstcSdif->SNINTST = 0x0100u; // Clear interrupt flag
        SdifDisableCardInt( pstcSdif );
...
/**
 ******************************************************************************
 ** \brief Enable the interrupt flag of SD Card Interrupt
 **
 ** \param pstcSdif         SDIF instance
 **
 ** \retval Ok
 ** \retval ErrorInvalidParameter If the following case matches:
 **                               - pstcSdif == NULL
 **
 ******************************************************************************/
en_result_t SdifEnableCardInt( volatile stc_sdifn_t* pstcSdif )
{
    if (NULL == pstcSdif)
    {
        return ErrorInvalidParameter ;
    ...
    pstcSdif->SNINTSTE_f.CARDINTS = 1;
    pstcSdif->SNINTSGE_f.CARDINTG = 1;
...
/**
 ******************************************************************************
 ** \brief Disable the interrupt flag of SD Card Interrupt
 **
 ** \param pstcSdif         SDIF instance
 **
 ** \retval Ok
 ...
 **
 ******************************************************************************/
en_result_t SdifDisableCardInt( volatile stc_sdifn_t* pstcSdif )
{
    if (NULL == pstcSdif)
    {
        return ErrorInvalidParameter ;
    ...
    pstcSdif->SNINTSTE_f.CARDINTS = 0;
    pstcSdif->SNINTSGE_f.CARDINTG = 0;
