...
static inline void adts_find_frame( int8_t* err, adtsparse_t* adtsparse )
{
                 ...
                 ( adtsp->search_status & SEARCH_CRC_FOUND      ) ) ) )
    {
             ...
             ( ( id2[ 1 ] & 0xF0 ) == 0xF0 ) )
        {
            ...
            if ( adts_hdr.fixed.profile >= ADTS_HDR_AAC_PROFILE_MAX )
            {
                LOG_ERR( 0, LOG_ALWAYS, "[ADTSPARSE][%s] ADTS ERROR: invalid profile\n",
                         adtsp->label );
                myerr = -1;
            }
    ...
    if ( NO_ERR == myerr )
    {
        ...
        if ( !search_complete_f )
        {
            if ( ( BUF_NUM <= ( adtsp->data_fifo_w_idx + 1 ) ) ||
                 ( BUF_RAW_SIZE <= ( adtsp->data_fifo[ adtsp->data_fifo_w_idx ].raw_idx + buf_len ) ) )
            {
