...
void bt_audio_sink_data_cb( wiced_bt_a2dp_sink_codec_t codec_type, wiced_bt_a2dp_sink_audio_data_t* p_audio_data )
{
    ...
    if(audio == NULL)
    {
        wiced_log_msg(WICED_LOG_ERR,"bt_audio_sink_data_cb: buffer allocation failed !\n");
        return;
    }
...
static void bt_audio_sink_control_cb( wiced_bt_a2dp_sink_event_t event, wiced_bt_a2dp_sink_event_data_t* p_data)
{
    switch(event)
    {
        ...
        {
            wiced_log_msg(WICED_LOG_INFO,"bt_audio_sink_control_cb:CONNECT EVENT \nstatus = %d\n", p_data->connect.result);

            if (p_data->connect.result == WICED_SUCCESS)
            {
                apollo_bt_remote_control_peer_address_set(p_data->connect.bd_addr);
                wiced_log_msg(WICED_LOG_INFO,"bt_audio_sink_control_cb: Remote Bluetooth Address: [%02X:%02X:%02X:%02X:%02X:%02X]\n",
        ...
        case WICED_BT_A2DP_SINK_DISCONNECT_EVT:
        {
            ...
            wiced_log_msg(WICED_LOG_INFO,"bt_audio_sink_control_cb:DISCONNECTED EVENT \nreason = %d \nRemote Bluetooth Address: [%02X:%02X:%02X:%02X:%02X:%02X]\n",
        ...
        {
            wiced_log_msg(WICED_LOG_INFO,"bt_audio_sink_control_cb:STREAM START EVENT \nstatus = %d\n", p_data->start.result);
        ...
        {
            wiced_log_msg(WICED_LOG_INFO,"bt_audio_sink_control_cb:STREAM SUSPEND EVENT \nstatus = %d\n", p_data->start.result);
            if ( g_p_apollo_bt_a2dp_sink->user_params.event_cbf != NULL )
            {
...
wiced_result_t apollo_bt_a2dp_sink_init( apollo_bt_a2dp_sink_init_params_t *params )
{
    ...
    wiced_action_jump_when_not_true(g_apollo_bt_a2dp_sink.is_initialized == WICED_FALSE, _exit,
                                    wiced_log_msg(WICED_LOG_ERR, "Apollo BT A2DP Sink: already initialized !\n"));
    ...
    wiced_action_jump_when_not_true(result == WICED_SUCCESS, _exit,
                                    wiced_log_msg(WICED_LOG_ERR, "Apollo BT A2DP Sink: wiced_rtos_init_event_flags() failed !\n"));
    ...
    wiced_action_jump_when_not_true(g_apollo_bt_a2dp_sink.p_event_data != NULL, _exit,
                                    wiced_log_msg(WICED_LOG_ERR, "Apollo BT A2DP Sink: BT service is not initialized !\r\n"));
    wiced_action_jump_when_not_true(g_apollo_bt_a2dp_sink.p_event_data->enabled.status == WICED_BT_SUCCESS, _exit,
                                    wiced_log_msg(WICED_LOG_ERR, "Apollo BT A2DP Sink: BT service has not successfully started !\n"));
...
#ifdef BT_AUDIO_USE_MEM_POOL
    wiced_log_msg(WICED_LOG_DEBUG0, "Apollo BT A2DP Sink: using BT MEMPOOL instead of malloc()\r\n");
    result = bt_buffer_pool_init(&g_p_apollo_bt_a2dp_sink->mem_pool, MEM_POOL_BUFFER_COUNT, MEM_POOL_BUFFER_SIZE);
    wiced_action_jump_when_not_true(result == WICED_SUCCESS, _exit,
                                    wiced_log_msg(WICED_LOG_ERR, "Apollo BT A2DP Sink: BT buffer pool init failed !\n"));
#endif

    result = bt_audio_decoder_context_init( g_p_apollo_bt_a2dp_sink );
    wiced_action_jump_when_not_true(result == WICED_SUCCESS, _exit,
                                    wiced_log_msg(WICED_LOG_ERR, "Apollo BT A2DP Sink: audio decoder init failed !\n"));
    ...
    if ( result != WICED_SUCCESS )
    {
        ...
        wiced_log_msg(WICED_LOG_ERR, "Apollo BT A2DP Sink: wiced_dct_read_lock(DCT_BT_CONFIG_SECTION) failed !!\n");
    }
    else
    {
        wiced_log_msg(WICED_LOG_INFO,"Apollo BT A2DP Sink: WICED DCT BT ADDR [%x:%x:%x:%x:%x:%x] \r\n",
    ...
    wiced_log_msg(WICED_LOG_INFO,"Apollo BT A2DP Sink: wiced_bt_dev_write_eir() result = 0x%x\n", (unsigned int)result);

    wiced_bt_dev_read_local_addr( g_p_apollo_bt_a2dp_sink->local_address );
    wiced_log_msg(WICED_LOG_INFO,"Apollo BT A2DP Sink: Local Bluetooth Address is [%02X:%02X:%02X:%02X:%02X:%02X]\n",
    ...
    wiced_log_msg( WICED_LOG_INFO, "Apollo BT A2DP Sink: wiced_bt_sdp_db_init() result (bool) = 0x%x\n", ( unsigned int ) result );
    ...
    wiced_log_msg(WICED_LOG_INFO, "Apollo BT A2DP Sink: discoverability result = 0x%x\r\n", (unsigned int)result);
    ...
    wiced_log_msg(WICED_LOG_INFO, "Apollo BT A2DP Sink: connectability result = 0x%x\r\n", (unsigned int)result);
    ...
    if ( result == WICED_SUCCESS )
    {
        ...
        {
            wiced_log_msg(WICED_LOG_INFO, "Apollo BT A2DP Sink: wiced_bt_remote_control_init() failed with result = 0x%x\n", (unsigned int)result);
        }
    }
    wiced_action_jump_when_not_true(result == WICED_SUCCESS, _exit,
                                    wiced_log_msg(WICED_LOG_ERR, "Apollo BT A2DP Sink: wiced_bt_a2dp_sink_init() failed with result = 0x%x!\n", (unsigned int)result));
...
wiced_result_t apollo_bt_a2dp_sink_deinit( void )
{
    ...
    wiced_action_jump_when_not_true( g_apollo_bt_a2dp_sink.is_initialized == WICED_TRUE, _exit, wiced_log_msg(WICED_LOG_ERR,"Apollo BT A2DP Sink: not yet initialized !\n") );
    ...
    {
        wiced_log_msg(WICED_LOG_INFO, "Apollo BT A2DP Sink: not CONNECTED ! !\n");
    }
    else
    {
        ...
        {
            wiced_log_msg(WICED_LOG_INFO, "Apollo BT A2DP Sink: got DISCONNECTED event !\n");
        }
        else
        {
            wiced_log_msg(WICED_LOG_INFO, "Apollo BT A2DP Sink: didn't get DISCONNECTED event (%d)!\n", (int)wait_result);
        }
    }
...
wiced_result_t apollo_bt_a2dp_sink_connect( void )
{
    ...
    wiced_action_jump_when_not_true( (g_apollo_bt_a2dp_sink.is_initialized == WICED_TRUE) && (g_apollo_bt_a2dp_sink.init_result == WICED_SUCCESS), _exit,
                                     wiced_log_msg(WICED_LOG_ERR,"Apollo BT A2DP Sink: not yet initialized !\n") );

    result = apollo_bt_nv_get_device_info_by_index(0, &out_device);
    wiced_action_jump_when_not_true( result == WICED_SUCCESS, _exit, wiced_log_msg(WICED_LOG_NOTICE,"Apollo BT A2DP Sink: no previously connected device exist !\n") );

    result = wiced_bt_a2dp_sink_connect(out_device.device_link.bd_addr);
    wiced_action_jump_when_not_true( result == WICED_SUCCESS, _exit, wiced_log_msg(WICED_LOG_NOTICE,"Apollo BT A2DP Sink: wiced_bt_a2dp_sink_connect() failed !\n"));
