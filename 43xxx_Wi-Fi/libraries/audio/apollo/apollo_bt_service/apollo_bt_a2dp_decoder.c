...
static wiced_result_t bt_audio_decoder_task( uint32_t arg )
{
    ...
    wiced_log_msg( WICED_LOG_INFO, "Apollo BT A2DP sink: starting decoder task...\r\n" );
    ...
    do
    {
        ...
        {
            wiced_log_msg( WICED_LOG_DEBUG0, "Apollo BT A2DP sink: wiced_rtos_wait_for_event_flags() failed with %d !\r\n", ( int )result );
            continue;
        }
        ...
        if ( flags_set & BT_AUDIO_CODEC_EVENT_DECODE )
        {
            while ( (a2dp_sink->decoder_thread_quit == WICED_FALSE) && (wiced_rtos_is_queue_empty( &a2dp_sink->decoder.queue ) != WICED_SUCCESS) )
            {
                ...
                {
                    wiced_log_msg( WICED_LOG_ERR, "Apollo BT A2DP sink: decoder not configured\r\n" );
                    break;
                }
                result = a2dp_sink->decoder.wiced_sbc_if->decode( );
                if ( result != WICED_SUCCESS )
                {
                    wiced_log_msg( WICED_LOG_DEBUG0, "Apollo BT A2DP sink: decoder failed with %d !\r\n", ( int )result );
                }
            }
        }
    ...
    wiced_log_msg( WICED_LOG_INFO, "Apollo BT A2DP sink: exiting decoder task !\r\n" );
...
wiced_result_t bt_audio_decoder_context_init( apollo_bt_a2dp_sink_t *a2dp_sink )
{
    ...
    wiced_action_jump_when_not_true( g_a2dp_sink == NULL, _exit, wiced_log_msg( WICED_LOG_ERR,"Apollo BT A2DP sink: audio decoder already initialized !\r\n" ) );
    g_a2dp_sink = a2dp_sink;

    a2dp_sink->decoder.wiced_sbc_if = wiced_get_codec_by_type( WICED_CODEC_SBC );
    wiced_action_jump_when_not_true(a2dp_sink->decoder.wiced_sbc_if != NULL, _exit, wiced_log_msg( WICED_LOG_ERR,"Apollo BT A2DP sink: wiced_get_codec_by_type() failed !\r\n" ) );

    a2dp_sink->decoder.wiced_sbc_if->init( &decoder_cbs, NULL );

    result = wiced_rtos_init_event_flags( &a2dp_sink->decoder.events );
    wiced_action_jump_when_not_true(result == WICED_SUCCESS, _exit, wiced_log_msg( WICED_LOG_ERR,"Apollo BT A2DP sink: wiced_rtos_init_event_flags() failed !\r\n" ) );
    ...
    wiced_action_jump_when_not_true(result == WICED_SUCCESS, _exit, wiced_log_msg( WICED_LOG_ERR,"Apollo BT A2DP sink: wiced_rtos_create_thread_with_stack() failed !\r\n" ) );
...
wiced_result_t bt_audio_decoder_context_deinit( apollo_bt_a2dp_sink_t *a2dp_sink )
{
    ...
    wiced_action_jump_when_not_true( g_a2dp_sink != NULL, _exit, wiced_log_msg( WICED_LOG_ERR,"Apollo BT A2DP sink: audio decoder not initialized !\r\n" ) );
    ...
    do
    {
        ...
        if ( result == WICED_SUCCESS )
        {
            ...
            wiced_log_msg( WICED_LOG_ERR,"Apollo BT A2DP sink: flushed %p from decoder queue !\n", a2dp_sink->decoder_input_buffer);
...
wiced_result_t bt_audio_write_to_decoder_queue( apollo_bt_a2dp_sink_t *a2dp_sink, bt_audio_codec_data_t* audio )
{
    ...
    if ( result != WICED_SUCCESS )
    {
        ...
        wiced_log_msg( WICED_LOG_ERR,"bt_audio_write_to_decoder_queue: wiced_rtos_push_to_queue() failed !\r\n" );
    }
    else
    {
...
wiced_result_t bt_audio_configure_decoder( apollo_bt_a2dp_sink_t *a2dp_sink, wiced_bt_a2dp_codec_info_t* decoder_config )
{
    ...
    {
        wiced_log_msg( WICED_LOG_ERR, "bt_audio_configure_decoder: unsupported codec !\r\n" );
    }
...
unsigned short bt_audio_decoder_alloc_output_buffer( int16_t** buffer, uint16_t length )
{
    ...
    if ( pcm == NULL )
    {
        wiced_log_msg( WICED_LOG_ERR,"bt_audio_decoder_alloc_output_buffer: buffer allocation failed !\r\n" );
        return 0;
    }
...
unsigned short bt_audio_decoder_read_encoded_data( uint8_t* data, uint16_t length )
{
    ...
    if ( g_a2dp_sink->decoder_input_buffer == NULL )
    {
        ...
        if ( result != WICED_SUCCESS )
        {
            wiced_log_msg( WICED_LOG_ERR, "bt_audio_decoder_task: wiced_rtos_pop_from_queue() failed !\r\n" );
            return 0;
        }
    }
