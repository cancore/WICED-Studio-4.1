...
wiced_result_t bt_buffer_pool_init( bt_buffer_pool_handle_t* pool_handle, uint16_t buffer_count, uint16_t data_size )
{
    ...
    if ( pool == NULL )
    {
        wiced_log_msg(WICED_LOG_ERR,"bt_buffer_pool_init: Pool allocation failed, out of heap space \n");
   ...
   pool->min_free_count = pool->free_count;
    ...
    else
    {
        *pool_handle = (bt_buffer_pool_handle_t)pool;
        wiced_log_msg(WICED_LOG_INFO,"bt_buffer_pool_init: Pool creation successful, pool handle %p \n", *pool_handle);
    }
...
wiced_result_t bt_buffer_pool_deinit( bt_buffer_pool_handle_t pool_handle )
{
    ...
    wiced_log_msg(WICED_LOG_INFO,"bt_buffer_pool_deinit: destroying pool %p \n", pool_handle);
...
#else
        wiced_log_msg(WICED_LOG_ERR,"bt_buffer_pool_allocate_buffer: failed. pool handle 0x%x, free_count %d\n", (int) pool_handle, pool->free_count);
#endif
    }
...
#endif
{
    ...
    //wiced_log_msg(WICED_LOG_ERR,"bt_buffer_pool_free_buffer: pool handle %p, buffer ptr %p free count after free %d\n", packet->pool, buffer, packet->pool->free_count);
