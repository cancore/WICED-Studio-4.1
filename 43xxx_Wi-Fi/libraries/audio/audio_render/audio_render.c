...
static inline wiced_result_t get_media_time(audio_render_t* audio, int64_t* media_time, wiced_bool_t* clock_set)
{
    ...
    else
    {
        ...
        {
            wiced_log_msg(WICED_LOG_ERR, "TIME WENT BACKWARDS!!!  Current: %d.%09d,  last: %d.%09d\n", PRINT_TIME(*media_time), PRINT_TIME(last_mtime));
        }
...
static int process_buffer_with_clock(audio_render_t* audio)
{
    ...
    if (result != WICED_SUCCESS)
    {
        wiced_log_msg(WICED_LOG_INFO, "Audio underrun detected (%lu bytes available)\n", audio->buffer_bytes_avail);
    ...
    {
        /* Potential underrun. */
        wiced_log_msg(WICED_LOG_ERR, "No audio buffer chunk available!\n");
        perform_audio_stop(audio);
        return buffer_bytes_avail;
    }
    ...
    {
        wiced_log_msg(WICED_LOG_INFO, "Audio buffer underrun occurred after fill!\r\n");
        perform_audio_stop(audio);
        return buffer_bytes_avail;
    }
...
static int process_buffer_without_clock(audio_render_t* audio)
{
    ...
    if (audio->clock_enable == WICED_TRUE && audio->clock_set == WICED_FALSE)
    {
        ...
        {
            wiced_log_msg(WICED_LOG_INFO, "Clock has been set. Enabling clock playback.\n");
            return buffer_bytes_avail;
        }
    }
...
static void process_audio_buffers(audio_render_t* audio)
{
    ...
    while (bytes_available >= audio->period_size && !audio->quit)
    {
    ...
    if (bytes_available < audio->period_size && audio->completion_notify)
    {
        /*
...
audio_render_ref audio_render_init(audio_render_params_t* params)
{
    ...
    if (attempts > 0)
    {
        wiced_log_msg(WICED_LOG_INFO, "Audio device took %d attempts to initialize\n", attempts + 1);
    }
...
wiced_result_t audio_render_configure(audio_render_ref audio, wiced_audio_config_t* config)
{
    ...
    wiced_log_msg(WICED_LOG_DEBUG0, "Using %d period buffers of size %d\n", audio->num_period_buffers, audio->period_size);
    ...
    if (result != WICED_SUCCESS)
    {
        wiced_log_msg(WICED_LOG_ERR, "Unable to create audio buffer %d (%d,%d)\n", result, audio->period_size, audio->num_period_buffers);
        return result;
    }
...
wiced_result_t audio_render_push(audio_render_ref audio, audio_render_buf_t *buf)
{
    ...
    if (buf == NULL)
    {
        ...
        wiced_log_msg(WICED_LOG_DEBUG0, "Completion notify flag set\n");
...
#if 0
    if (audio->media_time_set && audio->clock_enable == WICED_TRUE)
    {
        /*
        ...
        {
            wiced_log_msg(WICED_LOG_ERR, "Audio discontinuity detected. cur: %d:%09d  last: %d:%09d\r\n",
    ...
    if (audio->buffer_free_list != NULL)
    {
        ...
        if (audio->media_time_set == WICED_FALSE && audio->clock_enable == WICED_TRUE)
        {
            ...
            {
                wiced_log_msg(WICED_LOG_NOTICE, "media time - net is %d:%09d, PTS %d:%09d (clock_set: %d)\r\n", PRINT_TIME(mtime), PRINT_TIME(buf->pts), audio->clock_set);
            }
            else
            {
                wiced_log_msg(WICED_LOG_ERR, "Unable to retrieve base media time - disabling clock\r\n");
    ...
    else
    {
        ...
        {
            wiced_log_msg(WICED_LOG_ERR, "No free buffers in audio list (%lu used out of %lu)  bytes_queued %lu\n",
