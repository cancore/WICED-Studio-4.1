...
static FLAC__StreamDecoderReadStatus flac_read_callback(const FLAC__StreamDecoder *decoder, FLAC__byte buffer[], size_t *bytes, void *client_data)
{
    ...
    *bytes = bytes_to_copy;
    ...
    wiced_log_msg(WICED_LOG_DEBUG0, "flac_read_callback: copied:%ld\n", bytes_to_copy);
...
static FLAC__StreamDecoderWriteStatus flac_write_callback(const FLAC__StreamDecoder *decoder, const FLAC__Frame *frame, const FLAC__int32 * const buffer[], void *client_data)
{
    ...
    wiced_log_msg(WICED_LOG_DEBUG0, "flac_write_callback: ret %s\n", FLAC__StreamDecoderWriteStatusString[retval]);
...
static void flac_metadata_callback(const FLAC__StreamDecoder *decoder, const FLAC__StreamMetadata *metadata, void *client_data)
{
    ...
    switch(metadata->type)
    {
        case FLAC__METADATA_TYPE_STREAMINFO:    /* 0 */
            ...
            wiced_log_msg(WICED_LOG_INFO, "    sample rate    : %d Hz\n", dec->stream_sample_rate);
            wiced_log_msg(WICED_LOG_INFO, "    channels       : %d\n", dec->stream_num_channels);
            wiced_log_msg(WICED_LOG_INFO, "    bits per sample: %d\n", dec->stream_bits_per_sample);
            wiced_log_msg(WICED_LOG_INFO, "    total samples  : %lu\n", (uint32_t)dec->stream_total_samples);
            ...
            {
                wiced_log_msg(WICED_LOG_ERR, "We do not support FLAC sample_rate: %ld at this time <---!\n", dec->stream_sample_rate);
            ...
            {
                wiced_log_msg(WICED_LOG_ERR, "wiced_audio_render_configure: failed\n");
                client->decoder_done = WICED_TRUE;
            }
            break;
...
static void flac_error_callback(const FLAC__StreamDecoder *decoder, FLAC__StreamDecoderErrorStatus status, void *client_data)
{
    ...
    wiced_log_msg(WICED_LOG_ERR, "flac_error_callback: %s\n", FLAC__StreamDecoderErrorStatusString[status]);
}
...
static wiced_result_t flac_push_data_to_audio_render(audio_client_t* client, const FLAC__Frame *frame, const FLAC__int32 * const buffer[])
{
    ...
    wiced_log_msg(WICED_LOG_DEBUG1, "flac_push_data_to_audio_render:\n");
    wiced_log_msg(WICED_LOG_DEBUG1, "         blocksize: %d\n", frame->header.blocksize);
    wiced_log_msg(WICED_LOG_DEBUG1, "          channels: %d\n", frame->header.channels);
    wiced_log_msg(WICED_LOG_DEBUG1, "      this channel:(%d) %s\n", frame->header.channel_assignment, FLAC__ChannelAssignmentString[frame->header.channel_assignment]);
    wiced_log_msg(WICED_LOG_DEBUG1, "   bits per sample: %d\n", frame->header.bits_per_sample);
    wiced_log_msg(WICED_LOG_DEBUG1, "              rate: %d\n", frame->header.sample_rate);
    wiced_log_msg(WICED_LOG_DEBUG2, "         buffer[0]: %p\n", buffer[0]);
    wiced_log_msg(WICED_LOG_DEBUG2, "         buffer[1]: %p\n", buffer[1]);
    wiced_log_msg(WICED_LOG_DEBUG2, "         buffer[2]: %p\n", buffer[2]);
    ...
    if (frame->header.number_type == FLAC__FRAME_NUMBER_TYPE_SAMPLE_NUMBER)
    {
        ...
        {
            wiced_log_msg(WICED_LOG_DEBUG0, "Current sample correction: %lu -> %lu\n",
    ...
    while (samples_given_to_us > 0 && client->decoder_done == WICED_FALSE)
    {
        ...
        if (result != WICED_SUCCESS)
        {
            wiced_log_msg(WICED_LOG_ERR, "Error releasing audio buffer\n");
            return WICED_ERROR;
        }
...
static wiced_result_t flac_decoder_start(audio_client_t* client)
{
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "flac_decoder_start: FLAC decoder not initialized!\n");
        return WICED_ERROR;
    }
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "FLAC decoder flush failed\n");
    }

    if (FLAC__stream_decoder_finish(dec->flac_decoder) == false)
    {
        wiced_log_msg(WICED_LOG_ERR, "FLAC decoder finish failed\n");
    }
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "Unable to init FLAC stream\n");
        return WICED_ERROR;
    }
...
static wiced_result_t process_flac_data(audio_client_t* client)
{
    ...
    if (dec->flac_header_found)
    {
        ...
        if (ret == false)
        {
            ...
            audio_buf.flags = AUDIO_CLIENT_BUF_FLAG_EOS;
            audio_client_buffer_release(client, &audio_buf);
            wiced_log_msg(WICED_LOG_ERR, "FLAC__stream_decoder_process_single: ret = %d\n", ret);
            goto _errout;
        }
    }
    else
...
static void audio_client_flac_thread(uint32_t arg)
{
    ...
    wiced_log_msg(WICED_LOG_INFO, "audio_client FLAC decoder start\n");
    ...
    wiced_log_msg(WICED_LOG_INFO, "audio_player FLAC decoder exit\n");
...
wiced_result_t audio_client_flac_decoder_start(audio_client_t* client)
{
    ...
    if (client == NULL)
    {
        wiced_log_msg(WICED_LOG_ERR, "audio_client_flac_decoder_start: Bad ARG\n");
        return WICED_BADARG;
    }
    ...
    if (dec->flac_decoder == NULL)
    {
        wiced_log_msg(WICED_LOG_ERR, "Unable to create new FLAC decoder\n");
        free(dec);
        return WICED_ERROR;
    }
    ...
    if (result != WICED_SUCCESS)
    {
        wiced_log_msg(WICED_LOG_ERR, "audio_client_flac_decoder_start: wiced_rtos_create_thread failed %d\n", result);
        return WICED_ERROR;
    }
    else
    {
...
wiced_result_t audio_client_flac_decoder_stop(audio_client_t* client)
{
    ...
    wiced_log_msg(WICED_LOG_INFO, "audio_client_flac_decoder_stop\n");

    if (client == NULL)
    {
        wiced_log_msg(WICED_LOG_ERR, "audio_client_flac_decoder_stop: Bad ARG\n");
        return WICED_BADARG;
    }
...
wiced_result_t audio_client_flac_decoder_ioctl(struct audio_client_s* client, DECODER_IOCTL_T ioctl, void* arg)
{
    ...
    if (client == NULL)
    {
        wiced_log_msg(WICED_LOG_ERR, "audio_client_flac_decoder_ioctl: Bad handle\n");
        return WICED_BADARG;
    }
    ...
    if (dec == NULL || dec->tag != FLAC_TAG_VALID)
    {
        wiced_log_msg(WICED_LOG_ERR, "audio_client_flac_decoder_ioctl: Bad decoder handle\n");
        return WICED_BADARG;
    }
