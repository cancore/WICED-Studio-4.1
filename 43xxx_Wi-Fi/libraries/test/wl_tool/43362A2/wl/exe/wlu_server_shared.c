 ...
 *
 * $Id: wlu_server_shared.c,v 1.31.8.7 2011-02-04 02:30:21 jwang Exp $
 */
...
#endif //if defined(LINUX) || defined(vxworks)
	int skip;
	int download_flag = 0;
				...
				if (g_rem_ptr->msg.cmd == WLC_SET_VAR && buf_ptr &&
				   ...
				   !strncmp((const char *)buf_ptr,
					...
					DPRINT_INFO(OUTPUT, "SEND : cmd %d\t msg len %d\n",
					    g_rem_ptr->msg.cmd, g_rem_ptr->msg.len);
					DPRINT_DBG(OUTPUT, "error code: %d\n", err);
				}
			}
#if defined(LINUX)
			if (err == IOCTL_ERROR) {
...
#if defined(LINUX) || defined(SYMBIAN) || defined(TARGETOS_nucleus) || defined(MACOSX) || defined(TARGET_wiced)
...
#endif /* defined(LINUX) || defined(SYMBIAN) || defined(TARGETOS_nucleus) */
			g_rem_ptr->msg.flags = REMOTE_GET_IOCTL;
		} /* REMOTE_GET_IOCTL */
		DPRINT_DBG(OUTPUT, "RESP : cmd %d\t msg len %d\n",
		g_rem_ptr->msg.cmd, g_rem_ptr->msg.len);
#if defined(LINUX) || defined(vxworks)
