...
#define TCP_WINDOW_SIZE             WICED_TCP_WINDOW_SIZE
#endif
...
static wwd_interface_t interface;
        ...
        if ( sockets[a].available == WICED_TRUE )
        {
            ...
            switch ( type )
            {
                ...
                case SOCK_STREAM:
                    ...
                    /* Set option to maximum throughput delivery */
                    if ( nx_tcp_socket_create( &IP_HANDLE(interface), &free_socket->socket.tcp, "tcp_socket", NX_IP_MAX_DATA, NX_FRAGMENT_OKAY, NX_IP_TIME_TO_LIVE, TCP_WINDOW_SIZE, NX_NULL, NX_NULL ) != NX_SUCCESS )
                    {
                        return BSD_ERROR;
                    }
