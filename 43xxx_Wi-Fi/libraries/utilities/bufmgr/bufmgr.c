...
#define VALIDATE_BUF_HANDLE(c, h)                                                                   \
    ...
    {                                                                                               \
        wiced_log_msg(WICED_LOG_ERR, "invalid media data buffer handle\n");                         \
...
#define ASSERT_HANDLE(h)                                                                            \
    if (!(h))                                                                                       \
    {                                                                                               \
        wiced_log_msg(WICED_LOG_ERR, "invalid media data buffer handle (NULL)\n");                  \
        return BUFMGR_INVALID_BUF_HANDLE;                                                           \
    }
                               ...
                               uint32_t offset, uint32_t size, uint32_t timeout_ms)
{
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "invalid size\n");
                                 ...
                                 uint32_t offset, uint32_t size, uint32_t timeout_ms)
{
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "alloc_buf() failed (rc = 0x%08X)\n", rc);
        return rc;
    }
...
static bufmgr_status_t get_stats(objhandle_t hpool, bufmgr_stats_t* stats)
{
    ...
    if (!stats)
    {
        wiced_log_msg(WICED_LOG_ERR, "output statistic buffer is NULL\n");
                                   ...
                                   uint32_t handle_poolsize, uint32_t prepend_bytes, uint32_t alignment)
{
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "Buffer pool name too long, must be <= %d characters\n", MAX_POOL_NAME_SIZE - 6);
        return BUFMGR_INVALID_PARAM;
    }
    ...
    if (ctx == NULL)
    {
        wiced_log_msg(WICED_LOG_ERR, "allocating %s buffer pool context\n", name);
    ...
    if (priv == NULL)
    {
        wiced_log_msg(WICED_LOG_ERR, "allocating %s buffer pool private context\n", name);
    ...
    if (priv->buf_base_addr == NULL)
    {
        wiced_log_msg(WICED_LOG_ERR, "allocating %s buffer pool memory area\n", name);
        bufmgr_pool_free(ctx);
        return BUFMGR_MEM_ALLOC;
    }
    ...
    if (priv->buf_info == NULL)
    {
        wiced_log_msg(WICED_LOG_ERR, "allocating %s buffer pool\n", name);
        bufmgr_pool_free(ctx);
        return BUFMGR_MEM_ALLOC;
    }
    ...
    if (priv->buf_handle == NULL)
    {
        wiced_log_msg(WICED_LOG_ERR, "allocating %s handle pool\n", name);
        bufmgr_pool_free(ctx);
        return BUFMGR_MEM_ALLOC;
    }
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "wiced_rtos_init_semaphore(): %d\n", (int)result);
        bufmgr_pool_free(ctx);
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "wiced_rtos_init_mutex(): %d\n", (int)result);
        bufmgr_pool_free(ctx);
        return BUFMGR_UNKNOWN_ERR;
    }
