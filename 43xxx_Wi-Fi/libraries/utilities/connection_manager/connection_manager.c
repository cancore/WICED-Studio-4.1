...
static wiced_result_t p2p_discovery_enable                         ( void );
static wiced_result_t p2p_discovery_disable                        ( void );
...
static wiced_result_t check_mac_locally_administered_bit           ( void );

static wiced_result_t _p2p_go_start                                ( void );
static wiced_result_t _p2p_go_stop                                 ( void );
static wiced_result_t _p2p_gc_start                                ( wiced_bool_t reinvoke_group );
static wiced_result_t _p2p_gc_stop                                 ( void );
static wiced_result_t _wps_registrar_start                         ( void );
static wiced_result_t _wps_registrar_stop                          ( void );
static wiced_result_t _wps_enrollee_start                          ( wiced_bool_t check_stored_ap );
static wiced_result_t _wps_enrollee_stop                           ( void );
...
wiced_result_t connection_launch( connection_status_t connections )
{
    ...
    {
        result = _p2p_go_start();
        WPRINT_LIB_INFO(( " Launched P2P GO: %08X \n", result));
    }
    if (connections & CONNECTION_P2P_GC)
    {
        result = _p2p_gc_start(WICED_TRUE);
        WPRINT_LIB_INFO(( " Launched P2P GC: %08X \n", result));
    }
    if (connections & CONNECTION_P2P_GC_LEGACY)
    {
        result = _p2p_gc_start(WICED_FALSE);
        WPRINT_LIB_INFO(( " Launched P2P GC: %08X \n", result));
    }
    if (connections & CONNECTION_WPS_REGISTRAR)
    {
        result = _wps_registrar_start();
        WPRINT_LIB_INFO(( " Launched WPS Registar: %08X \n", result));
    }
    if (connections & CONNECTION_WPS_ENROLLEE)
    {
        result = _wps_enrollee_start(WICED_TRUE);
        WPRINT_LIB_INFO(( " Launched WPS Enrollee: %08X \n", result));
    }
    if (connections & CONNECTION_WPS_ENROLLEE_LEGACY)
    {
        result = _wps_enrollee_start(WICED_FALSE);
        WPRINT_LIB_INFO(( " Launched WPS Enrollee: %08X \n", result));
    }
...
wiced_result_t connection_kill( connection_status_t connections )
{
    ...
    if (connections & CONNECTION_WPS_REGISTRAR)
    {
        result = _wps_registrar_stop();
        WPRINT_LIB_INFO(( " Killed WPS Registar: %08X \n", result));
    }
    if (connections & CONNECTION_WPS_ENROLLEE)
    {
        result = _wps_enrollee_stop();
...
wiced_result_t _p2p_go_start( void )
{
    ...
    create_p2p_worker_thread(); // XXX check result
...
wiced_result_t _p2p_go_stop( void )
{
    wiced_result_t result = WICED_SUCCESS;
 ...
 * @return  WICED_SUCCESS for success, otherwise error
 */
wiced_result_t _p2p_gc_start( wiced_bool_t reinvoke_group )
{
    wiced_result_t result = WICED_SUCCESS;
    ...
    /* p2p_connection_request_handler will take a role for accepting invitation, group formation requests */
    if (p2p_discovery_enable() == WICED_SUCCESS)
    {
 ...
 * @return  WICED_SUCCESS for success, otherwise error
 */
wiced_result_t _p2p_gc_stop( void )
{
    wiced_result_t result = WICED_SUCCESS;
    ...
    /* p2p_connection_request_handler will take a role for accepting invitation, group formation requests */
    if (p2p_discovery_disable() == WICED_SUCCESS)
    {
 ...
 * @return  WICED_SUCCESS for success, otherwise error
 */
wiced_result_t _wps_registrar_start( void )
{
    wiced_result_t           result = WICED_SUCCESS;
 ...
 * @return  WICED_SUCCESS for success, otherwise error
 */
wiced_result_t _wps_registrar_stop( void )
{
    wiced_result_t           result = WICED_SUCCESS;
    /*
 ...
 * @return  WICED_SUCCESS for success, otherwise error
 */
wiced_result_t _wps_enrollee_start( wiced_bool_t check_stored_ap )
{
 ...
 * @return  WICED_SUCCESS for success, otherwise error
 */
wiced_result_t _wps_enrollee_stop( void )
{
    wiced_result_t           result = WICED_SUCCESS;
...
/*!
 ******************************************************************************
 * Start MCU Powersave Sleep
 *
 * @return  WICED_SUCCESS for success, otherwise error
 * @arg     int sleep_ms: how long the MCU goes to sleep, 0 means infinite.
 */
wiced_result_t mcu_powersave_sleep( uint32_t sleep_ms )
{
    wiced_result_t                  result = WICED_SUCCESS;
    platform_tick_powersave_mode_t  tick_mode = PLATFORM_TICK_POWERSAVE_MODE_TICKLESS_ALWAYS;
    platform_mcu_powersave_mode_t   mcu_mode = PLATFORM_MCU_POWERSAVE_MODE_SLEEP;
    uint16_t                        wait_wlan_res = 0;
    ...
    while ( 1 )
    {
        ...
        if ( PLATFORM_WLAN_POWERSAVE_IS_RES_UP() )
        {
            ...
            if ( ++wait_wlan_res > 1000 )
            {
                WPRINT_LIB_INFO (( "Refuse to sleep due to WLAN resources are still requested\n" ));
                break;
            }
...
static wiced_result_t p2p_connection_request_handler( void* connecting_device )
{
    ...
    if ( device->status == P2P_DEVICE_REQUESTED_TO_FORM_GROUP)
    {
        ...
        else
        {
            ...
            p2p_workspace.group_owner_intent = 0;
...
static wiced_result_t p2p_wps_result_handler( void* result )
{
    ...
    {
        WPRINT_LIB_INFO(( "p2p_wps_result_handler: WPS completed successfully\n" ));
    }
    else if ( *wps_result == WPS_PBC_OVERLAP )
    {
        WPRINT_LIB_INFO(( "p2p_wps_result_handler: PBC overlap detected - wait and try again\n" ));
    }
    else if ( *wps_result == WPS_ABORTED )
    {
        WPRINT_LIB_INFO(( "p2p_wps_result_handler: WPS aborted\n" ));
    }
    else
    {
        WPRINT_LIB_INFO(( "p2p_wps_result_handler: WPS timed out\n" ));
    }
