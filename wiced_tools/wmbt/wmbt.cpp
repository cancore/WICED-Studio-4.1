...
#include "tchar.h"
#include "wmbt_com.h"
#include "..\..\include\hci_control_api.h"
...
{
    printf("Usage: wmbt download COMx <hcd_pathname>\n");
}
...
static int execute_download(char *pathname)
{
    ComHelper SerialPort;
...
{
    printf ("Usage: wmbt le_receiver_test COMx <rx_frequency>\n");
    if (!full)
        return;
...
{
    printf ("Usage: wmbt le_transmitter_test COMx <tx_frequency> <data_length> <data_pattern>\n");
    if (!full)
        return;
...
{
    printf ("Usage: wmbt le_test_end COMx\n");
}
...
{
    printf ("Usage: wmbt tx_frequency_arm COMx <carrier on/off> <tx_frequency>\n");
...
{
    printf ("Usage: wmbt receive_only COMx <rx_frequency>\n");
    if (!full)
        return;
...
{
    printf("Usage: wmbt radio_tx_test COMx <bd_addr> <frequency> <modulation_type> <logical_channel> <bb_packet_type> <packet_length> <tx_power>\n");
    if (!full)
        return;
...
static int execute_radio_tx_test(char *bdaddr, int frequency, int modulation_type, int logical_channel, int bb_packet_type, int packet_length, int tx_power)
{
    ComHelper SerialPort;
    UINT32     params[6];
    BOOL      res;
...
{
    printf("Usage: wmbt radio_rx_test COMx <bd_addr> <frequency> <modulation_type> <logical_channel> <bb_packet_type> <packet_length>\n");
    if (!full)
        return;
    printf("                bd_addr: BD_ADDR of Tx device (6 bytes)\n");
...
static int execute_radio_rx_test(char *bdaddr, int frequency, int modulation_type, int logical_channel, int bb_packet_type, int packet_length)
{
    ComHelper SerialPort;
    UINT32     params[6];
    BOOL      res;
...
int _tmain(int argc, _TCHAR* argv[])
{
    ...
    if (argc >= 3)
        sscanf_s(argv[2], _T("COM%d"), &port_num);
                        ...
                        if ((logical_channel >= 0) && (logical_channel <= 1))
                        {
                            ...
                            if ((bb_packet_type >= 3) && (bb_packet_type <= 15))
                            {
                                ...
                                if ((packet_length >= 0) && (packet_length <= 0xffff))
                                {
                                    ...
                                    {
                                        return execute_radio_tx_test(argv[3], frequency, modulation_type, logical_channel, bb_packet_type, packet_length, tx_power);
                                    }
                                }
                            }
                        }
                    ...
                    if ((modulation_type >= 0) && (modulation_type <= 4))
                    {
                        ...
                        if ((logical_channel >= 0) && (logical_channel <= 1))
                        {
                            ...
                            if ((bb_packet_type >= 3) && (bb_packet_type <= 15))
                            {
                                ...
                                if ((packet_length >= 0) && (packet_length <= 0xffff))
                                {
                                    return execute_radio_rx_test(argv[3], frequency, modulation_type, logical_channel, bb_packet_type, packet_length);
                                }
                            }
                        }
                    }
