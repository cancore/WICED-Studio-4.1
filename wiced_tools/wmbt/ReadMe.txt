...
RF performance of the BCM20706 family of SoC Bluetooth Low Energy (BLE) devices. 
Each test sends a WICED HCI command to the device and then waits for the appro-
priate WICED HCI event from the device.

To run the tests:
  1. Configure the BCM20706 to run in the WICED HCI mode. The user MUST download
  ...
  2. Plug the BCM20706 into the computer and note the COM port assigned to 
...
BCM20706 by the PC. Replace “COM4” in each test example with the actual port 
assigned to the BCM20706 under test).
...
This command sends the WICED HCI command HCI_CONTROL_COMMAND_RESET to the BCM20706.
...
This test configures the BCM20706 to receive reference packets at a fixed 
...
The LE Transmitter Test configures the BCM20706 to send test packets at a 
fixed interval. External test equipment may be used to receive and analyze 
the reference packets. 

The frequency on which the BCM20706 transmits the packets  is passed as a 
...
This test configures the BCM20706 to turn the carrier ON or OFF. When the 
carrier is ON the device transmits an unmodulated pattern on the specified 
frequency at a specified power level.

The frequency to be used by the BCM20706 is passed as a parameter.
...
The example below turns the carrier ON and instructs the BCM20706 to transmit an 
...
This test configures the BCM20706 to transmit the selected data pattern which is on the specified frequency
...
the BCM20706 are passed as parameters.
...
The example below instructs the BCM20706 to transmit 0xAA 8-bit Pattern on the 2402 MHz and ACL Basic
...
This test issues a command to the BCM20706 to set radio to camp on a specified frequency.
While test is running the BCM20706 periodically sends reports about received packets.
...
BCM20706 will generate the statistics report of the Rx Test every second.

The example below instructs the BCM20706 to receive 0xAA 8-bit Pattern on the 2402 MHz and ACL Basic with DM1 packet type.
...
necessary to bring the BCM20706 out of the test mode. To execute another test, you must download the application
image again.
